...
class evenementDaos
{
    public static function getAllEvenementsByAsso(int $associationId): array
    {
        $sql = "SELECT * FROM evenement WHERE Association = :association_id";
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            $evenement->setId($row['Id']);
            $evenement->setTitre($row['Titre']);
            $evenement->setDateEvenement($row['Date_Evenement']); // Conversion automatique dans le setter
            $evenement->setLieu($row['lieu']);
            $evenement->setAssociation($row['Association']); // Assure-toi que cette colonne existe dans la base de données
