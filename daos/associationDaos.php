<?php

namespace daos;

use entities\association;
use daos\PdoBD as DaosPdoBD;
use PdoBD;
use DateTime;
use Exception;
use PDOException;

require_once "PdoBD.php";

class associationDaos
{
    public static function getAllAssoc($associationId = 0)
    {
        $associations = [];

        // Connexion à la base de données
        $db = DaosPdoBD::getInstance()->getMonPdo();
        $sql = "SELECT * FROM association";

        // Si un ID d'association est passé, on ajoute la condition WHERE
        if ($associationId > 0) {
            $sql .= " WHERE Id = ?";
        }

        try {
            // Préparation et exécution de la requête
            $stmt = $db->prepare($sql);

            // Si un ID est passé, on le lie à la requête
            if ($associationId > 0) {
                $stmt->bindParam(1, $associationId, \PDO::PARAM_INT);
            }

            $stmt->execute();

            // Récupération des résultats
            while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                $id = $row['Id'];
                $nom = $row['Nom'];
                $image = $row['image'];

                // Création de l'objet Association et ajout dans le tableau
                $association = new Association($id, $nom, $image);
                $associations[] = $association;
            }
        } catch (\PDOException $e) {
            error_log("Erreur lors de la récupération des associations : " . $e->getMessage());
        }

        return $associations;
    }
    public static function getAssociationById($associationId) {
        // Connexion à la base de données
        $db = DaosPdoBD::getInstance()->getMonPdo();
        $sql = "SELECT * FROM association WHERE Id = ?";
        
        try {
            // Préparation et exécution de la requête
            $stmt = $db->prepare($sql);
            $stmt->bindParam(1, $associationId, \PDO::PARAM_INT);
            $stmt->execute();
        
            // Vérifier si l'association existe
            if ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                $id = $row['Id'];
                $nom = $row['Nom'];
                $image = $row['image'];
        
                // Création de l'objet Association
                $association = new Association($id, $nom, $image);
                return $association;  // Retourner l'association trouvée
            } else {
                // Si aucune association n'est trouvée, retourner null ou une erreur
                return null;
            }
        } catch (\PDOException $e) {
            error_log("Erreur lors de la récupération de l'association par ID : " . $e->getMessage());
            return null;
        }
    }
    
}
